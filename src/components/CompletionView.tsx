...
export function CompletionView({ category, score, questionCount, onReview, onRetry, onLibrary }: CompletionViewProps) {
  ...
  return (
    <>
      ...
      <div className="completion-score"><span>{String(score).padStart(2, '0')}</span><small>/ {String(questionCount).padStart(2, '0')} RIGHT</small></div>
      <h1 className="focus-target" ref={headingRef} tabIndex={-1}>Good work.<br /><em>Keep the signal.</em></h1>
      <p className="completion-copy">You worked through all {questionCount} {category.name} cases. Review the full path again or choose another track when you’re ready.</p>
